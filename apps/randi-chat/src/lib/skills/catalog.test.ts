...
it("loadSkillBySlug resolves non-primary sources and parses frontmatter safely", async () => {
    const workspaceSkill = await loadSkillBySlug("privy-auth");
    const importedSkill = await loadSkillBySlug("mcp-builder");
    expect(workspaceSkill).toBeTruthy();
    expect(workspaceSkill?.source).toBe("Workspace");
    expect(workspaceSkill?.description ?? "").toMatch(/Privy/i);
